...
{
	return MyMatrix3{
		std::cosf(angleRadians), -std::sinf(angleRadians), 0.0,
		std::sinf(angleRadians), std::cosf(angleRadians), 0.0,
		0.0, 0.0, 1.0 }	;
}
...
{
	return MyMatrix3{
		std::cosf(t_angleRadians), 0.0, std::sinf(t_angleRadians),
		0.0 ,1.0 ,0.0,
		-std::sinf(t_angleRadians), 0.0, std::cosf(t_angleRadians)
	};
}
...
MyMatrix3 MyMatrix3::rotationX(const float t_angleRadians)
{
	return MyMatrix3{
		1.0 , 0.0 , 0.0,
		0.0 ,std::cosf(t_angleRadians) ,-std::sinf(t_angleRadians),
		0.0 , std::sinf(t_angleRadians), std::cosf(t_angleRadians)};
}
